...
def train_eval(train_dataset, dev_dataset, name):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # print(f"[SST-2] 训练中，第 {step + 1}/{len(train_loader)} 个 batch，loss={loss.item():.4f}")
        print(f"[SST-2] 训练中，第 {step + 1}/200 个 batch，loss={loss.item():.4f}")
        if step >= 199:  # 只跑前5个 batch 就停
            break
...
# ----------- 任务1：SST-2（单句情感分类） -----------
print("任务1：SST-2")
sst2_path = os.path.join(glue_path, 'SST-2')
sst2_train = pd.read_csv(os.path.join(sst2_path, 'train.tsv'), sep='\t')
sst2_dev = pd.read_csv(os.path.join(sst2_path, 'dev.tsv'), sep='\t')

sst2_train_set = GLUEDataset(sst2_train, tokenizer, sentence1_key='sentence')
sst2_dev_set = GLUEDataset(sst2_dev, tokenizer, sentence1_key='sentence')

train_eval(sst2_train_set, sst2_dev_set, name='SST-2')
